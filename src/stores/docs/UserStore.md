...
2. `sessionTools` - небольшой тулсет для работы с сессией;
   ##### Структура тулсета
    ...
    - `addChangingListener(callable)`: Метод для регистрации коллбэков, которые будут вызваны при каждом изменении
      состояния сессии.
      Принимает параметр `callable` - функция, которая будет вызываться;
      Не возвращает ничего, сохраняет переданные `callable` во внутреннем представлении хранилища;
