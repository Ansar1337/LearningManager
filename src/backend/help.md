...
- Доступные `actions`:
        ...
        - Структура ответа:
            ```
            {
              ...
              "data": [{
                ...
                "icon": URL, // Путь к иконке
                "bigIcon": URL, // Путь к хайрез иконке                 
              }, ...]
            }
            ```
        - Ошибки: Не предусмотрены.
        ...
        - **Структура ответа**:
          ```
          {
            ...
            "data": {
              ...
              "comments": [
                {
                  "senderId": INT, // ID отправителя комментария, 0 для системных сообщений
                  "senderName": STRING, // UserName отправителя комментария, "System" для системных сообщений
    ...
    - ### addHomeworkSubmission ###
        ...
        - ### downloadHomeworkFile ### 
            ...
            - **Структура ответа**:
                ...
                - В случае ошибки: Возвращается стандартный JSON-ответ с `status: "error"`
                  и `data: "описание ошибки"`.
        ...
        - ### launchUserCourseModuleTest ###
            - **Описание**: Запускает тест модуля курса для пользователя. Проверяет авторизацию пользователя и наличие
              указанного
              курса и модуля. Инициализирует тест, если не был достигнут лимит попыток или если предыдущая попытка
              прохождения
              теста
              была сделана более 30 дней назад. Возвращает сообщение о запуске теста, готовя его к прохождению
              пользователем.
            - **Параметры**:
                - `courseId`: INT, идентификатор курса.
                - `moduleId`: INT, идентификатор модуля.
            - **Структура ответа**:
                ```
                {
                  "status": STRING, // "success" в случае успешного запуска теста, "error" в случае ошибки
                  "data": STRING // Описание результата операции, например, "test launched" или сообщение об ошибке
                }
                ```
            - **Ошибки**:
                - **not logged in**: Пользователь не авторизован.
    ...
    - ### getUserCourseModuleTestQuestions ###
        - **Описание**: Возвращает вопросы активного теста модуля курса для пользователя. Этот метод следует
          непосредственно
          за `launchUserCourseModuleTest`, когда тест уже был успешно запущен. Проверяет авторизацию пользователя и
          наличие
          указанного курса и модуля. Возвращает вопросы теста без указания правильных ответов, готовые к отображению
          пользователю.
            - **Параметры**:
                - `courseId`: INT, идентификатор курса.
                - `moduleId`: INT, идентификатор модуля.
            ...
            - **Ошибки**:
                ...
                - **test not found**: Тест не найден.
                    ...
                    ```
                - **test not started**: Тест не был начат.
                  ```
                  {
                  "status": "error",
        ...
        - ### updateUserCourseModuleTest ###
            ...
            - **Параметры**:
                ...
                - `answers`: JSON, объект с ответами на вопрос, где ключи — это варианты ответа, а значения
                  указывают на
        ...
        - ### reviewUserCourseModuleTest ###
            - **Описание**: Проводит оценку завершенного теста модуля курса для пользователя. Проверяет, авторизован
              ли
              пользователь и существуют ли указанный курс и модуль. Если все условия удовлетворены и тест находится
              в
              состоянии "завершен" (`idle`), метод оценивает тест, вычисляя итоговый балл, количество ошибок и
              статус
              прохождения теста.
            - **Параметры**:
                - `courseId`: INT, идентификатор курса.
        ...
        - ### getUnreadMessages ###
            ...
            - **Ошибки**:
                - **not logged in**: Пользователь не авторизован.
                    ...
                    ```
                - **can't fetch user subscriptions**: Не удается получить информацию о подписках пользователя на
                  курсы или
                  модули.
                  ```
                  {
